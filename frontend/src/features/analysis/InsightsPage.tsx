...
export function InsightsPage(): JSX.Element {
  ...
  const {
    ...
    isEmpty,
    studyId,
  } = useValues(insightsLogic);
  const { rerunInsights, toggleFinding, setStudyId } =
    useActions(insightsLogic);
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        ...
        {insights?.status === "ready" && (
          <Button variant="secondary" size="sm" onClick={rerunInsights}>
      ...
      {!studyId && (
        <EmptyState
          ...
          title={t("insights.select_study")}
          description={t("insights.select_study_desc")}
        />
      )}
      {studyId && isLoading && <LoadingState message={t("insights.loading")} />}
      {studyId && isGenerating && <GeneratingState />}
      {studyId && !isLoading && isFailed && (
        <EmptyState
          icon={<Sparkles className="h-6 w-6" />}
          title={t("insights.generation_failed")}
          description={errorMessage || t("insights.generation_failed_desc")}
          action={{ label: t("insights.retry"), onClick: rerunInsights }}
        />
      )}
      {studyId && !isLoading && !isGenerating && !isFailed && isEmpty && (
        <EmptyState
          icon={<Lightbulb className="h-6 w-6" />}
          title={t("insights.no_insights")}
          description={t("insights.no_insights_desc")}
          action={{ label: t("insights.generate"), onClick: rerunInsights }}
        />
      )}
      ...
      {studyId && insights?.status === "ready" && (
        <>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <KpiCard
              label={t("insights.kpi_interviews")}
              value={String(insights.completed_interviews)}
              size="title"
              variant="card"
            />
            <KpiCard
              label={t("insights.kpi_avg_session")}
              value={`${insights.avg_session_minutes}m`}
              size="title"
              variant="card"
            />
            <KpiCard
              label={t("insights.kpi_topics")}
              value={String(insights.interview_topics?.length ?? 0)}
              size="title"
              variant="card"
            />
            <KpiCard
              label={t("insights.kpi_last_updated")}
              value={
          ...
          {insights.executive_summary && (
            <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
              <section className="rounded-merism-lg border border-[color:var(--merism-hairline)] bg-merism-surface p-6">
                <h2 className="mb-3 font-mono text-merism-caption uppercase tracking-merism-caps text-merism-text-subtle">
                  {t("insights.executive_summary")}
                </h2>
                <div className="text-merism-body leading-relaxed text-merism-text whitespace-pre-wrap">
                  {insights.executive_summary}
                </div>
                <p className="mt-4 border-t border-[color:var(--merism-hairline)] pt-3 font-mono text-merism-caption text-merism-text-subtle">
                  {t("insights.based_on", {
                    count: insights.completed_interviews,
                  })}
                </p>
              </section>
              <section className="rounded-merism-lg border border-[color:var(--merism-hairline)] bg-merism-surface p-6">
                ...
                {insights.interview_topics?.length > 0 ? (
                  <ul className="flex flex-col gap-3">
                    {insights.interview_topics.map((topic: any, i: number) => (
                      <li
                        key={i}
                        className="flex items-start gap-3 text-[15px] leading-relaxed text-merism-text"
                      >
          ...
          {highlights.length > 0 && (
            <section>
              <h2 className="mb-4 font-mono text-merism-caption uppercase tracking-merism-caps text-merism-text-subtle">
                {t("insights.highlights")}
              </h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {highlights.map((h: InsightHighlight) => (
          ...
          {findings.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-2">
                {findings.map((f: InsightFinding) => (
                  <FindingRow
                    key={f.id}
                    finding={f}
                    expanded={!!expandedFindings[f.id]}
  ...
  return (
    <Card className="flex flex-col gap-3 p-5">
      <div className="flex items-center gap-2">
        <Icon className="h-4 w-4 text-merism-accent" />
  ...
  return (
    <div className="rounded-merism-lg border border-[color:var(--merism-hairline)] bg-merism-surface">
      <button
        ...
        className="flex w-full items-center gap-3 px-5 py-4 text-left transition-colors hover:bg-merism-bg-subtle"
      >
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {finding.themes.map((th, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1.5 rounded-full bg-merism-bg-subtle px-3 py-1 text-merism-caption text-merism-text"
              >
          ...
          {finding.insight_nuggets.map((n, i) => (
            <div key={i} className="rounded-merism-md bg-merism-bg-subtle p-3">
              <div className="text-merism-title font-display font-[450] text-merism-text">
                {n.value}
                {n.unit && (
                  <span className="ml-1 text-merism-caption text-merism-text-muted">
                    {n.unit}
                  </span>
                )}
              </div>
              <div className="text-merism-caption text-merism-text-muted">
                {n.label}
              </div>
            </div>
          ))}
