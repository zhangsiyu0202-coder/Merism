...
export const insightsLogic = kea<insightsLogicType>([
      ...
      null as StudyInsightsData | null,
      {
        loadInsights: async () => {
          ...
          const res = await api.get(
            `/api/study-insights/?study=${values.studyId}`,
          );
    ...
    highlights: [
      [] as InsightHighlight[],
      {
        loadHighlights: async () => {
          ...
          const res = await api.get(
            `/api/insight-highlights/?insights=${values.insights.id}`,
          );
    ...
    findings: [
      [] as InsightFinding[],
      {
        loadFindings: async () => {
          if (!values.insights?.id) return [];
          const res = await api.get(
            `/api/insight-findings/?insights=${values.insights.id}`,
          );
          return (res as any).results ?? res;
        },
      },
  ...
  listeners(({ actions, values, cache }) => ({
    ...
    loadInsightsSuccess: () => {
      if (values.insights?.status === "generating") {
        ...
        actions.stopPolling();
        if (values.insights && values.insights.status === "ready") {
    ...
    rerunInsights: async () => {
      ...
      if (!values.insights?.id) {
        // No insights record yet — create one, which triggers generation
        await api.create(`/api/study-insights/`, { study: values.studyId });
        actions.loadInsights();
        return;
      }
  ...
  selectors({
    ...
    isFailed: [
      (s) => [s.insights],
      (insights: StudyInsightsData | null) => insights?.status === "failed",
    ],
    errorMessage: [
      (s) => [s.insights],
      (insights: StudyInsightsData | null) => insights?.error_message || "",
    ],
    isEmpty: [
      (s) => [s.insights],
      (insights: StudyInsightsData | null) =>
