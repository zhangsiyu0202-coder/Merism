...
export const reportDetailLogic = kea<reportDetailLogicType>([
  ...
  actions({
    ...
    addQuestion: (title: string, questionType: string) => ({
      title,
      questionType,
    }),
    startPolling: true,
  ...
  reducers({
    reportId: ["" as string, { setReportId: (_, { reportId }) => reportId }],
    activeSegment: [
      null as string | null,
      ...
      [] as ReportQuestion[],
      {
        loadQuestions: async () => {
          ...
          const res = await api.get(
            `/api/report-questions/?report=${values.reportId}`,
          );
    ...
    segments: [
      [] as ReportSegment[],
      {
        loadSegments: async () => {
          if (!values.reportId) return [];
          const res = await api.get(
            `/api/report-segments/?report=${values.reportId}`,
          );
          return (res as any).results ?? res;
        },
      },
  ...
  selectors({
    ...
    filteredQuestions: [
      ...
      (questions: ReportQuestion[], segment: string | null) =>
        segment
          ? questions.filter((q) => q.segment === segment || !q.segment)
          : questions,
    ],
  }),
]);
