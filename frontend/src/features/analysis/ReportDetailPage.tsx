...
import { Button, Card, Dialog, DialogContent, DialogTitle, Input, Tabs, TabsList, TabsTrigger } from "~/lib/merism"
        ...
        <div className="flex flex-col gap-6 p-6">
            <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <Button variant="secondary" size="sm" onClick={() => window.open(`/api/custom-reports/${reportId}/export_csv/`, "_blank")}>
                        <Download className="mr-1.5 h-3.5 w-3.5" />CSV
                    </Button>
                    <Button variant="primary" size="sm" onClick={generateReport}>
                        <Sparkles className="mr-1.5 h-3.5 w-3.5" />{t("insights.generate")}
                    </Button>
                </div>
            </div>
            ...
            {report?.is_public && (
                <div className="flex items-center gap-2 rounded-merism-md bg-merism-accent/5 px-4 py-2 text-merism-body-sm text-merism-accent">
                    ...
                    <button onClick={() => report?.share_url && navigator.clipboard.writeText(window.location.origin + report.share_url)} className="ml-auto underline">
                        {t("recruit.copy_link")}
                    </button>
                </div>
            )}
            ...
            {report?.ai_synthesis && (
                <Card className="p-5">
                    ...
                    <p className="text-merism-body-sm leading-relaxed text-merism-text">{report.ai_synthesis}</p>
                </Card>
            )}
            ...
            <div className="flex items-center gap-3">
                ...
                <Button variant="secondary" size="sm" onClick={() => setShowAddQ(true)}>
                    <Plus className="mr-1.5 h-3.5 w-3.5" />{t("outline.question_text")}
                </Button>
            </div>
            ...
            <Dialog open={showAddQ} onOpenChange={setShowAddQ}>
                <DialogContent>
                    ...
                    >
                        <Input value={newQTitle} onChange={(e) => setNewQTitle(e.target.value)} placeholder={t("reports.question_placeholder")} autoFocus />
                        <select
                            value={newQType}
                        ...
                        >
                            <option value="open_ended">{t("reports.question_types.open_ended")}</option>
                            <option value="multi_select">{t("reports.question_types.multi_select")}</option>
                            <option value="single_select">{t("reports.question_types.single_select")}</option>
                            <option value="rating">{t("reports.question_types.rating")}</option>
                            <option value="ranking">{t("reports.question_types.ranking")}</option>
                        </select>
                        <div className="flex justify-end gap-2">
                            <Button variant="secondary" size="sm" onClick={() => setShowAddQ(false)} type="button">{t("common.cancel")}</Button>
                            <Button variant="primary" size="sm" type="submit">{t("common.create")}</Button>
                        </div>
                    </form>
                </DialogContent>
            </Dialog>
        ...
        <Card className="overflow-hidden">
            <div className="border-b border-[color:var(--merism-hairline)] px-5 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                        <h3 className="text-merism-body-sm font-medium text-merism-text">{question.title}</h3>
                        <p className="text-merism-caption text-merism-text-muted">{typeLabels[question.question_type] ?? question.question_type}</p>
                    </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-2">
                                {question.themes.map((th, i) => (
                                    <div key={i} className="flex items-center justify-between rounded-merism-md bg-merism-bg-subtle px-3 py-2">
                                        <div>
                                            <span className="text-merism-body-sm font-medium text-merism-text">{th.name}</span>
                                            {th.description && <p className="text-merism-caption text-merism-text-muted">{th.description}</p>}
                                        </div>
                                        <span className="text-merism-caption text-merism-text-muted">{th.count}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-2">
                                {question.quotes.map((q, i) => (
                                    <blockquote key={i} className="border-l-2 border-merism-accent/40 pl-3 text-merism-body-sm italic text-merism-text-muted">
                                        &ldquo;{q.text}&rdquo;
                                        <cite className="mt-1 block text-merism-caption not-italic text-merism-text-subtle">&mdash; {q.source}</cite>
                                    </blockquote>
                                ))}
                            </div>
                        </div>
            ...
            {question.status === "generating" && (
                <div className="flex items-center gap-2 px-5 py-4 text-merism-body-sm text-merism-accent">
                    <Sparkles className="h-4 w-4 animate-pulse" />{t("reports.generating")}…
                </div>
            )}
        </Card>
    )
...
function QuestionStatus({ status }: { status: string }): JSX.Element {
    ...
    return (
        <span className={`rounded-full px-2 py-0.5 text-merism-caption ${styles[status] ?? ""}`}>{status}</span>
    )
}
