...
export const textInterviewLogic = kea<textInterviewLogicType>([
    ...
    actions({
        ...
        finalizeAssistant: (finalText: string, sessionStatus: string) => ({ finalText, sessionStatus }),
    ...
    listeners(({ actions, values }) => ({
        bootstrapFromUrl: () => {
            const match = window.location.pathname.match(/^\/interview\/([a-f0-9-]+)/i)
            const sid = match?.[1]
        ...
        sendMessage: async () => {
            ...
            try {
                const response = await fetch(`/api/sessions/${values.sessionId}/message/`, {
                    method: "POST",
                    credentials: "include",
...
function _parseSSE(block: string): { event: string; data: Record<string, unknown> } | null {
