...
import type { InterviewMode, InterviewTurn, LocalAttachment, Stimulus } from "./types"
...
export default function InterviewRoomPage() {
    ...
    const { connect, attachCapture, clear: clearStream, sendTextInput } = useActions(
        voiceStreamLogic,
    )
        ...
        <div className="flex h-screen items-center justify-center bg-merism-bg-subtle p-6">
            ...
            >
                ...
                <p className="mb-6 text-sm text-merism-text-muted">
                    Choose how you'd like to take part. You'll speak; the AI moderator will guide
                    the conversation.
                </p>
...
function ModeTile({ active, icon, label, description, onSelect }: ModeTileProps) {
    return (
        <button
            type="button"
            onClick={onSelect}
...
function LiveRoom({
    ...
    if (!sessionId) {
        return (
            <div className="flex h-screen items-center justify-center bg-merism-bg-subtle">
                <span className="text-sm text-merism-text-muted">Starting session…</span>
            </div>
        )
    }
...
function RoomHeader({ phase }: { phase: string }) {
    return (
        <header className="relative flex h-14 shrink-0 items-center justify-center border-b border-[color:var(--merism-hairline)] bg-merism-surface px-4">
            ...
            <span className="absolute right-4 text-xs text-merism-text-muted">{phase}</span>
        </header>
    )
}
...
function Waveform({ micLevel }: { micLevel: number }) {
    ...
    const bars = Array.from({ length: BAR_COUNT }, (_, i) => {
        ...
        const height = 12 + Math.min(88, micLevel * 300 * (0.6 + 0.4 * Math.abs(phase)))
        return height
    })
    return (
        <div
...
function CameraPreview() {
    ...
    useEffect(() => {
        ...
        navigator.mediaDevices
            ...
            .catch((err) => setError(err instanceof Error ? err.message : "Camera denied."))
    ...
    if (error) {
        return (
            <span className="text-sm text-merism-text-muted">Camera unavailable · {error}</span>
        )
    }
    ...
    return (
        <>
            {stimulus.kind === "image" ? (
                ...
                <iframe src={stimulus.url} title={stimulus.title} className="h-full w-full" />
            )}
            <button
                type="button"
                        ...
                        <>
                            ...
                            {agentCaption && (
                                <div className="rounded-merism-md bg-merism-bg-subtle px-3 py-2 text-sm text-merism-text">
                                    {agentCaption}
                                    <span aria-label="Generating" className="ml-1 inline-flex gap-1 align-middle">
...
function AttachmentArea({ attachments, onAttach, onRemove }: AttachmentAreaProps) {
...
function TextOnlyRoom({ sessionId, turns, onStart, onSendText }: TextOnlyRoomProps) {
    useEffect(() => {
