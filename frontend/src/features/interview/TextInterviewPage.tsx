...
export default function TextInterviewPage(): JSX.Element {
  const { sessionId, turns, draft, isSending, isCompleted, error } =
    useValues(textInterviewLogic);
  const { setDraft, sendMessage, bootstrapFromUrl } =
    useActions(textInterviewLogic);
          ...
          <ul className="flex flex-col gap-4">
            {turns.map((t, i) => (
              ...
              >
                ...
                >
                  {t.text ||
                    (t.streaming ? <em className="opacity-60">…</em> : "")}
                </div>
              </li>
            ))}
            <div ref={endRef} />
