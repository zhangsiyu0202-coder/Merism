...
export default function ParticipantEntryPage(): JSX.Element {
  ...
  const { setSlug, startSession, submitConsent } = useActions(
    participantEntryLogic,
  );
  const { context, nextStep, contextLoading, errorCode } = useValues(
    participantEntryLogic,
  );
  ...
  if (!slug)
    return <FullscreenMessage title="链接无效" body="链接格式不正确。" />;
  if (contextLoading && !context)
    return <FullscreenMessage title="加载中…" illustration="loading-time" />;
      ...
      <div className="mx-auto max-w-lg">
        <header className="mb-8 flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-merism-sm bg-merism-accent text-white">
            <span className="font-display text-merism-body-sm font-[600]">
              M
            </span>
          </div>
          <span className="font-display text-merism-subtitle font-[500]">
            Merism
          </span>
        </header>
...
function StepView({
  ...
  if (step === "dropped")
    return (
      <FullscreenMessage
        title="感谢你的关注"
        body="本研究需要不同的参与者画像，感谢你的时间。"
        illustration="chill-time"
      />
    );
  ...
  return (
    <article className="rounded-merism-lg bg-merism-surface p-8 text-center shadow-merism-card ring-1 ring-[color:var(--merism-hairline)]">
      <Illustration
        name="fast-internet"
        size="xl"
        className="mx-auto mb-6 text-merism-text"
      />
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-merism-bg p-6">
      <div className="flex max-w-sm flex-col items-center gap-4 rounded-merism-lg bg-merism-surface p-10 text-center shadow-merism-card ring-1 ring-[color:var(--merism-hairline)]">
        ...
        <h1 className="font-display text-merism-h2 font-[500] text-merism-text">
          {title}
        </h1>
        {body && (
          <p className="text-merism-body text-merism-text-muted">{body}</p>
        )}
      </div>
    </main>
  );
...
function errorCopy(errorCode: string | null): { title: string; body: string } {
  switch (errorCode) {
    case "not_found":
      return {
        title: "链接无效",
        body: "找不到此链接，请检查 URL 或联系研究员获取新链接。",
      };
    case "link_closed":
    case "study_closed":
