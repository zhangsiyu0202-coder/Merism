...
async function _getJson(url: string): Promise<any> {
    ...
    if (!res.ok) throw Object.assign(new Error(body.error_code || `HTTP ${res.status}`), { body, status: res.status })
    return body
}
...
export const participantEntryLogic = kea<participantEntryLogicType>([
    ...
    loaders(({ values }) => ({
        context: [
            null as ParticipantContext | null,
            {
                loadContext: async () => {
                    ...
                    const body = await _getJson(`/i/${values.slug}/${previewQs}`)
        ...
        screenerQuestions: [
            ...
            {
                loadScreener: async () => {
                    const body = await _getJson(`/i/${values.slug}/screener/`)
    ...
    listeners(({ actions, values }) => ({
        ...
        submitConsent: async () => {
            try {
                const body = await _postJson(`/i/${values.slug}/consent/`)
        ...
        startSession: async () => {
            try {
                const body = await _postJson(`/i/${values.slug}/start/`)
                if (body.session_id) {
