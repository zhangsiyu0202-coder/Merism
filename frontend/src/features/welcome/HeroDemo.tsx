...
const SCRIPT: Turn[] = [
  ...
  {
    role: "participant",
    text: "大概是上周三吧,用了一段时间觉得不值。",
    delay: 800,
  },
  { role: "ai", text: '"不值"这个感觉很具体。能展开说说吗?', delay: 700 },
  {
    role: "participant",
    text: "主要是导出报告那块,每次都要等很久。",
    delay: 800,
  },
    ...
    if (ti >= SCRIPT.length) {
      ...
      if (ci === 0) {
        ...
        id = window.setTimeout(() => {
          setVis((p) => {
            const n = [...p];
            n[n.length - 1] = { role: cur.role, text: cur.text.slice(0, ci) };
            return n;
          });
          setCi(ci + 1);
        }, 30);
  ...
  return (
    <div className="relative h-[440px] w-full max-w-[420px] overflow-hidden rounded-[24px] bg-white shadow-[0_24px_60px_-20px_rgba(120,80,40,0.18)] ring-1 ring-[color:rgba(120,80,40,0.08)]">
      <div className="flex items-center gap-2 border-b border-[color:rgba(120,80,40,0.08)] px-5 py-3">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-[color:rgba(120,80,40,0.15)]" />
        <span className="h-2.5 w-2.5 rounded-full bg-[color:rgba(120,80,40,0.15)]" />
        <span className="ml-3 font-mono text-[11px] uppercase tracking-[0.14em] text-[#967864]">
          Merism · Interview
        </span>
      </div>
      <div
      ...
      >
        {vis.map((t, i) => {
          const typing =
            i === vis.length - 1 &&
            ti < SCRIPT.length &&
            ci <= (SCRIPT[ti]?.text.length ?? 0);
          const ai = t.role === "ai";
          return (
            <div
              key={i}
              className={ai ? "flex justify-start" : "flex justify-end"}
            >
              <div
                className={
                  "max-w-[86%] rounded-[14px] px-4 py-2.5 text-[14px] leading-[1.55] " +
                  (ai
                    ? "bg-[color:rgba(120,80,40,0.06)] text-[#2A1C10]"
                    : "bg-[var(--merism-accent)] text-white")
                }
              >
                {t.text}
                {typing && (
                  <span className="ml-0.5 inline-block h-[14px] w-[2px] animate-pulse bg-current align-middle" />
                )}
              </div>
            </div>
          );
