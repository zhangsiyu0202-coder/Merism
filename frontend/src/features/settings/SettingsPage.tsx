...
import { GlossarySection } from "./cleaning/GlossarySection";
...
type SettingsSectionKey = "profile" | "workspace" | "llm-gateway" | "glossary";
...
export default function SettingsPage(): JSX.Element {
  ...
  return (
    <div className="flex flex-col gap-[var(--spacing-merism-section-y)]">
      <PageTopBar title={t("settings.title")} lede={t("settings.lede")} />
        ...
        return (
          ...
          >
            ...
            <Icon
              className={
                "h-4 w-4 shrink-0 " +
                (active ? "text-merism-accent" : "text-merism-text-subtle")
              }
              strokeWidth={1.6}
            />
...
function ProfileSection({
  ...
  user: ReturnType<typeof useValues>["user"];
}): JSX.Element {
  const { t } = useTranslation();
  if (!user) {
    return (
