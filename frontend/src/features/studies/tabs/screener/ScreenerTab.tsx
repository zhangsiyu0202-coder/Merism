            ...
            >
              ...
              >
                <div className="flex flex-col gap-3">
                  {questions.map((q, i) => (
                    <ScreenerCard key={q.id} index={i} question={q} />
                  ))}
                </div>
              </SortableContext>
            </DndContext>
      ...
      {questions.map((q, i) => (
        ...
        >
          ...
          <span className="truncate">
            {q.text || (
              <span className="italic text-merism-text-subtle">(empty)</span>
            )}
          </span>
        </a>
      ))}
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          {question.type !== "free_text" && (
            <Tag
              variant={
                question.pass_option_indices.length === 0 ? "outline" : "accent"
              }
              size="sm"
            >
  ...
  const stats = useMemo<LiveStat[]>(() => {
    ...
    return [
      { label: "Questions", value: total },
      {
        ...
        value: withCriteria,
        hint: total
          ? `${Math.round((withCriteria / total) * 100)}%`
          : undefined,
