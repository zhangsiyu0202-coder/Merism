        ...
        <table className="w-full border-collapse text-merism-label">
          <thead>
            {table.getHeaderGroups().map((hg) => (
              <tr
                key={hg.id}
                className="border-b border-[color:var(--merism-hairline)]"
              >
  ...
  return [
    col.accessor((row) => row.participant_name || "", {
      ...
      cell: (info) => {
        ...
        if (name) {
          return (
            <span className="text-merism-body-sm text-merism-text">{name}</span>
          );
        }
        return (
    ...
    col.display({
      ...
      cell: ({ row }) => {
        ...
        if (!s.started_at)
          return <span className="text-merism-text-subtle">—</span>;
    ...
    status === "in_progress"
      ...
      : status === "completed"
        ...
        : status === "abandoned" || status === "excluded"
          ...
          : "neutral";
  return (
    <Tag variant={variant as "accent" | "success" | "danger" | "neutral"}>
      {status}
    </Tag>
  );
}
