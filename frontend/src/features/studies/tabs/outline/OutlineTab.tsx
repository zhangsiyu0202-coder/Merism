...
const RATING_SCALE_OPTIONS: Array<{
  value: GuideQuestion["scaleRange"];
  label: string;
}> = [
  { value: "1-5", label: "1 - 5" },
  { value: "1-7", label: "1 - 7" },
...
const RANDOMIZATION_OPTIONS: Array<{
  value: SectionRandomizationMode;
  label: string;
}> = [
...
export default function OutlineTab(): JSX.Element {
  ...
  const { study: _study } = useValues(studyLogic);
  ...
  const {
    ...
    saveGuide: (revision: number) => void;
  };
      ...
      <div className="grid min-h-0 flex-1 lg:grid-cols-[19rem_minmax(0,1fr)_18rem]">
        ...
        <main className="min-h-0 overflow-y-auto px-6 py-4">
          <DynamicQuestionEditor
            ...
            questionNumber={
              selectedQuestionId
                ? (questionIndexById.get(selectedQuestionId) ?? null)
                : null
            }
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-[color:var(--merism-hairline)] px-4 py-4">
          <div className="sticky top-0 z-10 -mx-4 -mt-4 mb-4 border-b border-[color:var(--merism-hairline)] bg-merism-surface px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0">
                <div className="font-mono text-merism-caption uppercase tracking-merism-caps text-merism-text-subtle">
                  提纲
                </div>
                <div className="mt-0.5 text-merism-label">
                  {isSavingGuide ? (
                    <span className="text-merism-text-muted">保存中…</span>
                  ) : isDirty ? (
                    <span className="text-merism-warning">有未保存改动</span>
                  ) : (
                    <span className="text-merism-text-subtle">已保存</span>
                  )}
                </div>
              </div>
              <Button
                variant="primary"
                size="sm"
                onClick={() => saveGuide(draftRevision)}
                disabled={!isDirty || isSavingGuide}
            ...
            {guideError ? (
              <div className="mt-2 text-merism-caption text-merism-danger">
                {guideError}
              </div>
            ) : null}
          </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center justify-between gap-3 pb-3">
        <div>
          <h2 className="text-sm font-medium text-merism-text">
            Guide Outline
          </h2>
            ...
            <div className="flex flex-col gap-2">
              {section.questions.map((question) => (
                <SortableQuestionRow
                  ...
                  onDuplicate={() =>
                    onDuplicateQuestion(section.id, question.id)
                  }
    ...
    <div ref={setNodeRef} style={style}>
      <div
        ...
        className={cn(
          ...
          selected ? "bg-merism-bg-subtle" : "hover:bg-merism-bg-subtle",
        )}
      >
        <button
          {...attributes}
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pt-4">
        <div className="flex flex-col gap-6">
          <EditorField
            label={question.type === "task" ? "Task text" : "Question text"}
            helper="Main text shown to participants."
          >
            <Textarea
              value={question.text}
              onChange={(e) =>
          ...
          <EditorField
            label="Question type"
            helper="Switching type swaps the configuration block below."
          >
            <Select
              value={question.type}
              onValueChange={(value) =>
          ...
          <div className="rounded-merism-lg border border-[color:var(--merism-hairline)] bg-merism-surface p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <StimulusPicker
                existingIds={question.stimulusIds}
                onSelect={(stimId) =>
                  onAddStimulus(section.id, question.id, stimId)
                }
              />
            </div>
        ...
        <Select
          ...
          onValueChange={(value) =>
            onUpdateQuestion(sectionId, question.id, {
              probingAmount: value as ProbeAmount,
            })
          }
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="rounded-merism-md border border-[color:var(--merism-hairline)] bg-merism-bg-subtle px-3 py-2 text-sm text-merism-text-muted">
        Task questions append a system prompt and are meant for goal-directed
        actions, not free-form discussion.
      </p>
...
function MultipleChoiceFields({
  ...
  const handleDragEnd = (event: DragEndEvent): void => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const fromIndex = question.options.findIndex(
      (option) => option.id === active.id,
    );
    const toIndex = question.options.findIndex(
  ...
  return (
    <label className="flex items-start gap-3 rounded-merism-md border border-[color:var(--merism-hairline)] bg-merism-surface px-3 py-2">
      ...
      <span className="min-w-0 flex-1">
        ...
        {helper && (
          <span className="mt-1 block text-xs leading-relaxed text-merism-text-muted">
            {helper}
          </span>
        )}
      </span>
...
function buildScalePreview(
  scaleRange: GuideQuestion["scaleRange"],
): JSX.Element[] {
  const values =
    scaleRange === "0-10"
      ? [0, 2, 4, 6, 8, 10]
...
function buildSummary(
  ...
  const questionsWithStimulus = sections.reduce(
    (sum, section) =>
      sum +
      section.questions.filter((question) => question.stimulusIds.length > 0)
        .length,
    0,
  );
  const estimatedMinutes = Math.max(
    5,
    Math.round((totalQuestions * 2.25) / 5) * 5,
  );
  return {
    totalQuestions,
...
// ── Stimulus Picker (inline popover) ──────────────────────────
...
function StimulusPicker({
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api
      .list<StimulusBrief>("/api/stimuli/", { study: studyId })
      .then((res: { results?: StimulusBrief[] }) =>
        setStimuli(res.results ?? []),
      )
      .catch(() => {})
  ...
  const available = stimuli.filter((s) => !existingIds.includes(s.id));
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-64 rounded-merism-lg border border-[color:var(--merism-hairline)] bg-merism-surface p-2 shadow-merism-pop">
          {loading ? (
            <p className="px-3 py-2 text-xs text-merism-text-muted">Loading…</p>
          ) : available.length === 0 ? (
            <p className="px-3 py-2 text-xs text-merism-text-muted">
              {stimuli.length === 0
                ? "No stimuli uploaded yet. Go to Stimuli tab to upload."
                : "All stimuli already attached."}
            </p>
          ) : (
            available.map((s) => (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-merism-md px-3 py-2 text-left text-sm text-merism-text hover:bg-merism-bg-subtle"
              >
...
// ── Loading skeleton for the section list (Bug 3 fix) ─────────────
//
// Renders a few muted bars matching the rough shape of the section /
// question list. Used while the first ``loadGuide`` API call is in
// flight so users don't see the STARTER_SECTIONS English placeholder
// flash before the real outline swaps in.
//
// REMOVED 2026-05-25: the muted-on-muted color choice rendered the
// skeleton effectively invisible against the panel background, so
// users saw a blank panel under the "Guide Outline / Sections &
// questions" header and reported the page as broken. The flash of
// STARTER content (~200 ms) is the lesser evil and is back.
