...
import { Button, ChatPanel, Sidebar, Tag, type ChatMessage } from "~/lib/merism"
...
export function OutlineReviewSidebar({ onApplyChanges }: OutlineReviewSidebarProps) {
    const { open, messages, isSending } = useValues(outlineReviewLogic)
    const { close, send, markApplied } = useActions(outlineReviewLogic)

    const chatMessages: ChatMessage[] = messages.map((m) => toChatMessage(m, onApplyChanges, markApplied))
    ...
    return (
        ...
        >
            <ChatPanel
                ...
                footer={
                    <span>
                        AI proposes changes; you accept one-by-one. Nothing is written to the
                        outline until you click Apply.
                    </span>
                }
...
function toChatMessage(
    ...
    return {
        ...
        content: m.text,
        footer:
            m.role === "assistant" && m.proposed_changes && m.proposed_changes.length > 0 ? (
                <ProposedChanges
