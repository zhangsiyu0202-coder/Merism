...
export type ProbeAmount = "light" | "standard" | "deep"
export type SectionRandomizationMode = "ordered" | "shuffle_sections" | "shuffle_questions"
...
export const DEFAULT_SECTION_RANDOMIZATION_MODE: SectionRandomizationMode = "ordered"

const QUESTION_TYPE_DEFAULTS: Record<GuideQuestionType, Omit<GuideQuestion, "id" | "text">> = {
    conversational: {
        type: "conversational",
        probingAmount: "standard",
        probingInstructions: "Probe for context, examples, and moments of friction when needed.",
        allowSkip: true,
    ...
    task: {
        type: "task",
        probingAmount: "standard",
        probingInstructions: "Use probing only after the participant finishes the task.",
        allowSkip: true,
    ...
    rating: {
        type: "rating",
        probingAmount: "light",
        probingInstructions: "Ask for rationale after the score when the rating is ambiguous.",
        allowSkip: true,
        allowDynamicProbe: false,
        stimulusIds: [],
        selectionType: "single_select",
    ...
    number_input: {
        type: "number_input",
        probingAmount: "light",
        probingInstructions: "Keep probes short; the answer is mainly quantitative.",
        allowSkip: true,
        allowDynamicProbe: false,
        stimulusIds: [],
        selectionType: "single_select",
    ...
    return [
        ...
        },
        {
            ...
            questions: [
                ...
                buildQuestion(
                    "rating",
                    "How confident did you feel while doing that?",
                ),
            ],
        },
        {
...
function resetQuestionType(question: GuideQuestion, type: GuideQuestionType): GuideQuestion {
    ...
    return {
        ...
        options: defaults.options.map((option) => ({ ...option, id: makeId("option") })),
...
function firstQuestionLocation(sections: GuideSection[]): { sectionId: string; questionId: string } | null {
...
function pickGuideForStudy(
    guides: InterviewGuideRecord[],
    studyId: string,
): InterviewGuideRecord | null {
    const matching = guides.filter((guide) => guide.study === studyId)
    if (matching.length === 0) {
        return null
    }
    return (
        matching.find((guide) => guide.is_current) ??
        [...matching].sort(
...
function scheduleAutosave(triggerSave: () => void): void {
...
export const outlineEditorLogic = kea<outlineEditorLogicType>([
    ...
    actions({
        ...
        updateSectionTitle: defineActionCreator((sectionId: string, title: string) => ({
            sectionId,
            title,
        })),
        ...
        selectQuestion: defineActionCreator((sectionId: string, questionId: string) => ({
            sectionId,
            questionId,
        })),
        clearSelection: true,
        addQuestion: defineActionCreator(
            (sectionId: string, questionType: GuideQuestionType, questionId: string) => ({
                sectionId,
                questionType,
                questionId,
            }),
        ...
        removeQuestion: defineActionCreator((sectionId: string, questionId: string) => ({
            sectionId,
            questionId,
        })),
        ...
        updateQuestion: defineActionCreator(
            (sectionId: string, questionId: string, patch: Partial<GuideQuestion>) => ({
                sectionId,
                questionId,
                patch,
            }),
        ...
        setQuestionType: defineActionCreator(
            (sectionId: string, questionId: string, questionType: GuideQuestionType) => ({
                sectionId,
                questionId,
                questionType,
            }),
        ...
        addQuestionOption: defineActionCreator(
            (sectionId: string, questionId: string, optionId: string, text: string) => ({
                sectionId,
                questionId,
                optionId,
                text,
        ...
        moveQuestionOption: defineActionCreator(
            (sectionId: string, questionId: string, fromIndex: number, toIndex: number) => ({
                sectionId,
                questionId,
                fromIndex,
                toIndex,
        ...
        setSectionRandomizationMode: defineActionCreator((mode: SectionRandomizationMode) => ({
            mode,
        })),
        loadGuide: true,
        loadGuideSuccess: defineActionCreator((guide: InterviewGuideRecord | null) => ({
            guide,
        })),
    ...
    reducers({
        sections: [
            STARTER_SECTIONS,
            {
                loadGuideSuccess: (state, { guide }) => (guide ? guide.sections : state),
                saveGuideSuccess: (_state, { guide }) => guide.sections ?? STARTER_SECTIONS,
                ...
                moveSection: (state, { fromIndex, toIndex }) => arrayMove(state, fromIndex, toIndex),
                ...
                duplicateQuestion: (state, { sectionId, questionId, newQuestionId }) =>
                    state.map((section) => {
                        ...
                        const cloned = cloneQuestion(section.questions[index] as GuideQuestion)
                ...
                moveQuestion: (state, { sectionId, fromIndex, toIndex }) =>
                    state.map((section) =>
                        section.id === sectionId
                            ? { ...section, questions: arrayMove(section.questions, fromIndex, toIndex) }
                            : section,
                    ),
                ...
                removeStimulusFromQuestion: (state, { sectionId, questionId, stimulusId }) =>
                    updateQuestion(state, sectionId, questionId, {
                        stimulusIds: (findQuestion(state, sectionId, questionId)?.stimulusIds ?? []).filter(
                ...
                updateQuestionOption: (state, { sectionId, questionId, optionId, text }) =>
                    updateQuestion(state, sectionId, questionId, {
                        options: (findQuestion(state, sectionId, questionId)?.options ?? []).map((option) =>
                            option.id === optionId ? { ...option, text } : option,
                        ),
                    }),
                removeQuestionOption: (state, { sectionId, questionId, optionId }) =>
                    updateQuestion(state, sectionId, questionId, {
                        options: (findQuestion(state, sectionId, questionId)?.options ?? []).filter(
                            (option) => option.id !== optionId,
                        ),
                    }),
                moveQuestionOption: (state, { sectionId, questionId, fromIndex, toIndex }) =>
                    updateQuestion(state, sectionId, questionId, {
                        options: arrayMove(findQuestion(state, sectionId, questionId)?.options ?? [], fromIndex, toIndex),
                    }),
    ...
    listeners(({ actions: a, values }) => ({
        ...
        loadGuide: async () => {
            ...
            try {
                const response = await api.list<InterviewGuideRecord>("/api/guides/", {
                    limit: 200,
                })
                const guide = pickGuideForStudy(response.results ?? [], values.studyId)
        ...
        saveGuide: async ({ revision }) => {
            ...
            if (values.isSavingGuide) {
                return
            }
            const payload = {
                study: values.studyId,
                version: values.loadedGuideVersion,
                is_current: true,
                language: values.loadedGuideLanguage,
                sections: values.sections,
            }
            try {
                const guide = values.loadedGuideId
                    ? await api.replace<InterviewGuideRecord>(
                          `/api/guides/${values.loadedGuideId}/`,
                          payload,
                      )
                    : await api.create<InterviewGuideRecord>("/api/guides/", payload)
                a.saveGuideSuccess(guide, revision)
                if (values.draftRevision > revision) {
        ...
        ...AUTOSAVE_ACTIONS.reduce<Record<string, () => void>>((acc, actionName) => {
            acc[actionName] = () => {
                if (!values.studyId || values.isLoadingGuide) {
                    return
                }
        ...
        removeQuestion: ({ sectionId, questionId }) => {
            if (values.selectedQuestionId !== questionId || values.selectedSectionId !== sectionId) {
                return
            }
            const section = values.sections.find((item) => item.id === sectionId)
            if (!section) return
            ...
            if (!question) return
            const remaining = section.questions.filter((item) => item.id !== questionId)
            if (remaining[0]) {
                a.selectQuestion(sectionId, remaining[0].id)
                return
            }
            const fallback = firstQuestionLocation(values.sections.filter((item) => item.id !== sectionId))
            if (fallback) {
        ...
        removeSection: ({ sectionId }) => {
            ...
            const remaining = values.sections.filter((section) => section.id !== sectionId)
