...
export const conceptBlocksLogic = kea<conceptBlocksLogicType>([
  ...
  actions({
    ...
    addConcept: (
      blockId: string,
      payload: { label: string; stimulus: string; notes?: string },
    ) => ({
      blockId,
      payload,
    }),
    ...
    blocks: [
      [] as ConceptBlockRow[],
      {
        loadBlocks: async () => {
          ...
          if (!studyId) return [];
          const res = await api.list<ConceptBlockRow>("/api/concept-blocks/", {
            study: studyId,
          });
          return res.results ?? [];
        },
  ...
  listeners(({ actions, values }) => ({
    ...
    addConcept: async ({ blockId, payload }) => {
      ...
      try {
        await api.create("/api/concepts/", {
          ...
          label:
            payload.label.trim() ||
            `Concept ${String.fromCharCode(65 + nextRank)}`,
          rank: nextRank,
          notes: payload.notes ?? "",
        });
        actions.loadBlocks();
    ...
    removeConcept: async ({ conceptId }) => {
      try {
        ...
        actions.setError(
          e instanceof Error ? e.message : "Remove concept failed",
        );
      }
    },
