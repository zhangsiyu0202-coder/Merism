...
import { Button, Input, SectionLabel, Tag } from "~/lib/merism"
    ...
    return (
        <div className="flex flex-col gap-6">
            ...
            {blocksLoading && blocks.length === 0 ? (
                ...
                <div className="rounded-merism-lg border border-dashed border-merism-border bg-merism-surface p-10 text-center text-sm text-merism-text-muted">
                    No concept blocks yet. Create one to run side-by-side comparative concept
                    testing.
                </div>
            ) : (
                <div className="flex flex-col gap-6">
...
function BlockCard({ block }: { block: ConceptBlockRow }) {
    const { setRotation, deleteBlock, reorderConcept } = useActions(conceptBlocksLogic)
    ...
    return (
        <section className="flex flex-col gap-4 rounded-merism-lg bg-merism-surface ring-1 ring-[color:var(--merism-hairline)] shadow-merism-card p-6">
            <header className="flex flex-wrap items-center gap-3">
                ...
                <Tag variant="outline">
                    {block.concept_count} {block.concept_count === 1 ? "concept" : "concepts"}
                </Tag>
                <div className="ml-auto flex items-center gap-3">
                    <div className="flex items-center gap-2">
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                setRotation(block.id, e.target.value as Rotation)
                            }
                            className="rounded-merism-md bg-merism-bg-subtle ring-1 ring-[color:var(--merism-hairline)] px-2 py-1 text-xs text-merism-text focus:border-merism-accent focus:outline-none"
                        >
...
function ConceptRowCard({ blockId, concept }: { blockId: string; concept: ConceptRow }) {
...
function AddConceptForm({ blockId }: { blockId: string }) {
    ...
    useEffect(() => {
        if (!study?.id) return
        setLoading(true)
        api.list<StimulusBrief>("/api/stimuli/", { study: study.id })
    ...
    return (
        ...
        >
            ...
            <select
                ...
                className="flex-1 min-w-56 rounded-merism-md border border-[color:var(--merism-hairline-strong)] bg-merism-surface px-3 py-2 text-sm text-merism-text focus:border-merism-accent focus:outline-none"
                aria-label="Stimulus"
            >
                <option value="">
                    {loading
