...
export function CrossSessionThemesPanel({ studyId }: Props): JSX.Element {
    ...
    useEffect(() => {
        setLoading(true)
        api.get<{ results: Theme[] }>("/api/themes/", { study: studyId })
            .then((r) => setThemes(r.results ?? (r as unknown as Theme[])))
            .finally(() => setLoading(false))
    }, [studyId])

    if (loading) {
        return <p className="text-merism-text-muted text-merism-body-sm">Loading themes…</p>
    }
        ...
        return (
            <div className="rounded-merism-lg border border-merism-border bg-merism-surface p-6">
                <p className="text-merism-body text-merism-text-muted">
                    No cross-session themes yet. Themes emerge after 8+ quotes
                    have been extracted across multiple sessions.
                </p>
            </div>
        )
    }
