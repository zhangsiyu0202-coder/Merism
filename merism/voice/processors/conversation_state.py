...
class ConversationItem:
    id: str                        # response_id for assistant; frame.name for user
    role: Role
    text: str = ""
    truncated: bool = False
    original_text: str = ""         # kept for diagnostics
...
class ConversationState(FrameProcessor):
    ...
    # Qwen-TTS zh default rate — ~4.5 chars/second → 0.0045 chars/ms.
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection) -> None:
        ...
        if isinstance(frame, TranscriptionFrame):
            # Close any open assistant item first (shouldn't normally
        ...
        if isinstance(frame, LLMFullResponseEndFrame):
            # Seal the item — original_text now frozen.
    ...
    def _apply_truncation(self, response_id: str, played_ms: int) -> None:
        # Find the matching assistant item — search backwards so the
        # most recent match wins if somehow IDs collide.
