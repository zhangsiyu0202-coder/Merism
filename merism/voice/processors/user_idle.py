...
Pipecat-style idle detection: an N-second timer resets every time we
see a ``UserStartedSpeakingFrame``. When the timer elapses we synthesise
a ``TranscriptionFrame("(idle)")`` upstream so the LLM produces a gentle
prompt ("Are you still there?").

Useful for interview UX — 10 s of silence is a natural moment for the
moderator to re-engage without feeling pushy.
"""
...
class UserIdleDetector(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection) -> None:
        ...
        if isinstance(frame, UserStartedSpeakingFrame):
            ...
            self._prompts_fired = 0       # reset once user speaks again
            self._cancel_timer()
            await self.push_frame(frame, direction)
            return
        ...
        if isinstance(frame, BotStoppedSpeakingFrame):
            self._bot_speaking = False
            self._reset_timer()            # start idle countdown once bot finishes
            await self.push_frame(frame, direction)
            return
    ...
    async def _fire_on_idle(self) -> None:
        ...
        # Inject as if the user said "(silent)" so the LLM generates a
